...
- **`feature/outline-mode`**: PaperSpecimen v2.0 - WIP (modalità outline vettoriale)
...
## Stato Progetto: ✅ v1.0 COMPLETATO | 🚧 v2.0 WIP
...
## 🚧 v2.0 Obiettivi: Modalità Outline Vettoriale

### Feature Target
**Toggle bitmap ↔ outline** con long press pulsante centrale (BtnP)

### Modalità Outline: Cosa Visualizzare
...
- **Off-curve points**: cerchi vuoti con bordo (~6-8px) - control points Bézier
...
- **Info tecnica**:
  ...
  - Tipo curve (Quadratic/Cubic Bézier)
- **Background**: bianco o grigio chiaro per contrasto
- **Footer**: stesso layout v1.0 (font name, Unicode codepoint)

### Accuratezza
- ✅ **100% fedele ai dati font originali** (no approssimazioni)
- ✅ Usa FreeType `FT_Outline_Decompose` per accesso diretto ai vettori
- ✅ Coordinate precise derivate da FT_Face (stesso scaling 375px del bitmap)
- ✅ Tipo curva corretto: quadratic (TrueType) vs cubic (PostScript/OpenType)

### Obiettivi Implementazione v2.0

#### 🎯 Fase 1: Accesso FT_Face (PRIORITY)
...
#### 🎯 Fase 2: Parsing Outline con FreeType
- [ ] Implementa callback functions per `FT_Outline_Decompose`:
  ...
  - [ ] `outlineConicTo(FT_Vector* control, FT_Vector* to, void* user)` - quadratic Bézier
  - [ ] `outlineCubicTo(FT_Vector* c1, FT_Vector* c2, FT_Vector* to, void* user)` - cubic Bézier
...
#### 🎯 Fase 3: Rendering Outline su Display
...
#### 🎯 Fase 4: Toggle Interaction (Long Press)
...
#### 🎯 Fase 5: Testing e Refinement
- [ ] Test su glifi semplici (A-Z): contours puliti
- [ ] Test su glifi complessi (@, &, ß): molti punti e curve
- [ ] Test ghosting outline mode (linee sottili più sensibili)
- [ ] Test toggle rapido bitmap ↔ outline (full refresh overhead)
- [ ] Verifica memoria: outline parsing non deve saturare heap
- [ ] Serial debug: log num_contours, num_points, curve types

#### 🎯 Fase 6: Polish e Ottimizzazioni
...
- **EASY**: Rendering primitives (linee, cerchi già disponibili in M5EPD)
...
- **COMPLEX**: Outline rendering performante (molti drawLine su e-ink può essere lento)

### Effort Estimate
- **Tempo stimato**: 3-5 ore (se tutto va liscio)
- **Codice stimato**: 250-400 righe (parsing + rendering + interaction)
- **Rischio**: MEDIO (accesso FT_Face è il punto critico)

### Use Case Professionale
Target utente: **Type designer, Typographer, Font enthusiast**
- ✅ Vedere costruzione reale del glifo (non approssimazione)
- ✅ Confrontare strategie design tra font (minimal points vs dettagliati)
- ✅ Capire come curve sono costruite (smooth vs sharp corners)
- ✅ Specimen tecnico oltre che visivo
